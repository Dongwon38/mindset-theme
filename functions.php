...
function fwd_widgets_init() {
	...
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar-secondary', 'fwd' ),
			'id'            => 'sidebar-2',
			'description'   => esc_html__( 'Add widgets here.', 'fwd' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
...
function fwd_scripts() {
	// Handle, Source, Dependencies, Version Number, Media
	...
	// enqueue to top files on everypage
	wp_enqueue_script (
		'totop',
		get_template_directory_uri() . '/js/totop.js',
		array(), 
		'20240827',
	...
	if ( is_front_page() ) {
		...
		wp_enqueue_script (
			...
			array( 'swiper-scripts' ), // dependency
			'20240827',
			array( 'strategy' => 'defer')
		);
	}
